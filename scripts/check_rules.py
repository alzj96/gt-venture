...
# (闸, 问的是什么, 对应的卡（相对 references/ 的路径，可以带通配符）, 覆盖到什么程度)
# 规则库迁进 knowledge/ 之后文件名和目录都变了，按路径认，不按文件名认——
# 按文件名认的话，迁走的那一格会报「规则文件不在」，而卡明明就在 knowledge/ 里。
#
# 「事」「地」两格原先写的是「整格空白」「抖音全没有」——B5–B26 做完门类骨架卡和
# 平台卡之后这两句就假了。说少了也是说错：模型读到「整格空白」会跳过库、直接联网，
# 把本来有逐字原文的许可说成「不知道」。所以这两格改成按通配符数卡，
# 一张都数不到才打 ✗；说明里照实写「覆盖了什么、没抓什么、没有的去哪儿查」。
GATES = [
    ("一、人", "这个人能不能做这件事", "knowledge/规则-主体资格.md",
     "覆盖：在编教师 / 公务员 / 事业单位 / 在校学生 / 在职副业 / 职务成果 / 个人接单的税。\n"
     "     未覆盖：医生、律师、会计师、军人、国企任职、外籍、失信被执行人。"),
    ("二、事", "这件事本身要不要许可", "knowledge/门类-*.md",
     "覆盖：国民经济行业分类 20 个门类的骨架卡（每个大类一节，许可的法律、行政法规逐字原文），\n"
     "     加横向规则卡（网络交易、直播、广告、个人信息、AI、食品网络销售、预付式消费、电商登记、特许经营、健康证、价格、定金）。\n"
     "     没抓：部门规章和地方规定（资质标准、各省设置标准、各地办法）；经营范围规范表述目录的明细。\n"
     "     先 kb_lookup.py 查；库里没有的说不知道：给类别名 + 市场监管部门/12345 的入口。"),
    ("三、地", "在哪个渠道上线，那个渠道要什么", "knowledge/平台-*.md",
     "类目表逐栏核过的只有微信小程序（高频那几类 + 个人主体类目）。\n"
     "     抖音：用户服务协议几条核过，抖店、团购规则待证；美团点评、小红书、快团团、淘宝拼多多：只有法规路由，平台规则一条没核到。\n"
     "     快手、视频号、支付宝、App 商店：没有卡。网站备案看门类 I 的 I-R2。"),
    ("四、钱", "钱怎么收、要不要办执照", "knowledge/钱-执照收款税票.md",
     "覆盖：市场主体登记、「零星小额」那条线的原文（电子商务经营者登记 D1、D5）、税务登记、收款码、小规模纳税人、发票（M1–M7）。\n"
     "     没抓：增值税起征点的具体数额和阶段性减免、收款码限额的现行口径、对公账户细则、跨境收款。\n"
     "     另见 knowledge/规则-预付式消费.md（预付款，不挑行业）、规则-定金订金押金.md 与 E9（个人接单的税）。"),
]
# 取标题行作为条目名，找不到就退回文件名
...
def scan(refs: Path, max_age: int, data_max_age: int = 180) -> int:
    ...
    for gate, question, fname, note in GATES:
        ...
        if hits:
            count = f"（{len(hits)} 份）" if any(c in fname for c in "*?[") else ""
            print(f"  · {gate}：{question}{count}")
            print(f"     {note}")
        else:
    ...
    extra = sorted(p.name for p in files
                   if not any(fnmatch(p.name, Path(g[2]).name) for g in GATES))
    if extra:
